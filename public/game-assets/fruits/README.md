...
| `litchi.png` | Litchi |
| `dates.png` | Dates |
| `bomb.png` | Bomb (instant game over if sliced) |

**Guidelines:**
- Square PNG, transparent background, ~256x256px.
- Keep the fruit centered and filling most of the frame — it gets scaled to fit its in-game radius automatically.
- To add a brand-new fruit type (not just replace an existing one), add an entry to `FRUITS` in `src/components/game/gameAssets.ts` with a `key`, `emoji` fallback, `color`, `points`, `radius`, and a `spritePath` pointing at a new file here.
