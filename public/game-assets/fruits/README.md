...
- Use your own photos/art or properly licensed stock (no visible watermarks, e.g. from a Dreamstime/Shutterstock/Getty preview) — those aren't cleared for use in a shipped product.

**Slicing:** when a real sprite is loaded here, the engine automatically pre-crops it into left/right halves so a slice splits the actual image (not just a copy of the whole mushroom) — no extra setup needed.
