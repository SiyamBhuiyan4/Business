...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const { name, icon, color, investment } = await request.json();
    ...
    const business = await prisma.business.update({ where: { id: params.id }, data: { name: name.trim(), slug, icon: icon || existing.icon, color: color || existing.color, ...(investment !== undefined ? { investment: Number(investment) } : {}) } });
